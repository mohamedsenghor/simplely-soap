...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
    ...
    @Query(
        value = "select payment from Payment payment left join fetch payment.initiator",
        countQuery = "select count(payment) from Payment payment"
    )
    Page<Payment> findAllWithToOneRelationships(Pageable pageable);

    @Query("select payment from Payment payment left join fetch payment.initiator")
    List<Payment> findAllWithToOneRelationships();

    @Query("select payment from Payment payment left join fetch payment.initiator where payment.id =:id")
